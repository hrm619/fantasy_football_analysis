    ...
    for pos in positions:
        ...
        for start_rank, end_rank in tiers[pos]:
            if len(pos_df) >= end_rank:
                ...
                if end_rank < len(pos_df) and start_rank > 1:
                    next_tier_avg = pos_df.iloc[end_rank : end_rank + 5][
                        "Half_PPR"
                    ].mean()
                    prev_tier_avg = pos_df.iloc[start_rank - 6 : start_rank - 1][
                        "Half_PPR"
                    ].mean()
                    tier_stats["Next_Tier_Diff"] = (
                        tier_stats["Avg_Points"] - next_tier_avg
                    )
                    tier_stats["Prev_Tier_Diff"] = (
                        prev_tier_avg - tier_stats["Avg_Points"]
                    )

                tier_results.append(tier_stats)
            else:
                logger.warning(
                    f"Not enough {pos} players for tier {start_rank}-{end_rank}"
                )
        ...
        for i in range(len(pos_df) - 1):
            diff = {
                ...
                "Point_Diff": pos_df.iloc[i]["Half_PPR"]
                - pos_df.iloc[i + 1]["Half_PPR"],
                "Pct_Diff": (
                    (pos_df.iloc[i]["Half_PPR"] / pos_df.iloc[i + 1]["Half_PPR"] - 1)
                    * 100
    ...
    for pos in positions:
        ...
        if len(pos_df) > 0:
            ...
            if len(pos_df) >= start_count:
                ...
                logger.warning(f"Not enough {pos} players for startable analysis")
            ...
            if not pos_df.empty:
                ...
                if len(pos_df) >= tier_size:
                    ...
                    stat = {
                        ...
                        "Top_Score": (
                            pos_df.iloc[0]["Half_PPR"] if len(pos_df) > 0 else 0
                        ),
                        "Top_To_Tier_End_Diff": (
                            pos_df.iloc[0]["Half_PPR"]
